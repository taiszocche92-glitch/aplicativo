...
7. FUNCIONA TANTO DA PASTA RAIZ QUANTO DA PASTA 'correção nomes'

MELHORIAS NA VERSÃO 2.2:
- Detecção automática de diretório de execução
- Ajuste automático de caminhos para banco de nomes e pasta alvo
- Compatibilidade com execução da raiz ou da pasta 'correção nomes'
...
Autor: Kilo Code
Data: 2025-08-29
Versão: 2.2 - Detecção automática de caminhos
"""
...
class PatientDataCleaner:
    ...
    def _create_identification_field(self, name: str, age: int, gender: str,
        ...
        """Cria o campo de identificação do paciente no formato 'Nome, Y anos, Z, W e K'."""
        # Criar lista de valores na ordem específica
        values_list = [name, f"{age} anos", occupation, marital_status]
        
        # Adicionar procedência se disponível
        if origin:
            values_list.append(origin)

        # Formatar como "Nome, Y anos, Z, W e K"
        if len(values_list) == 1:
            formatted_info = values_list[0]
        elif len(values_list) == 2:
            formatted_info = f"{values_list[0]} e {values_list[1]}"
        else:
            # Para 3 ou mais valores: "valor1, valor2, valor3 e valor4"
            formatted_info = ", ".join(values_list[:-1]) + f" e {values_list[-1]}"

        return {
            "contextoOuPerguntaChave": "IDENTIFICAÇÃO DO PACIENTE",
            "informacao": formatted_info
        }
    ...
    def _clean_existing_identification(self, info_text: str) -> str:
        ...
        if has_colon_format:
            ...
            found_formatnome_match = False
            for padrao in formatnome_padroes:
                match = re.search(padrao, info_text) # Usar info_text completo
                if match:
                    grupos = match.groups()
                    extracted_values['nome'] = grupos[0].strip()
                    extracted_values['idade'] = grupos[1].strip() # Não adicionar " anos" aqui, pois o _create_identification_field já adiciona
                    extracted_values['genero'] = grupos[2].strip() if len(grupos) > 2 else ""
                    extracted_values['ocupacao'] = grupos[3].strip() if len(grupos) > 3 else ""
                    extracted_values['estado_civil'] = grupos[4].strip() if len(grupos) > 4 else ""
                    found_formatnome_match = True
                    break # Se encontrou um padrão, para de procurar

            if not found_formatnome_match:
                for line in lines:
        ...
        # Construir lista de valores na ordem desejada: "Nome, Y anos, Z, W e K"
        values_list = []

        # Ordem específica: nome, idade, ocupação, estado civil, procedência
        # Gênero é omitido por padrão (apenas incluído em temas LGBT)
        order = ['nome', 'idade', 'ocupacao', 'estado_civil', 'procedencia']
    ...
    def process_file(self, file_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                instrucoes = data['instrucoesParticipante']
                descricaoCasoCompleta = instrucoes['descricaoCasoCompleta']
            except KeyError as e:
                if "parkinsonismo_parkinson_plus.json" in file_path:
                    return False, "Arquivo parkinsonismo_parkinson_plus.json ignorado devido a KeyError."
                else:
                    return False, f"Arquivo {file_path} não tem estrutura completa (falta instrucoesParticipante ou descricaoCasoCompleta)"
            ...
            if has_identification:
                ...
                for info in info_array:
                    if info.get('contextoOuPerguntaChave') == 'IDENTIFICAÇÃO DO PACIENTE':
                        ...
                        # Atualizar o campo 'informacao' com a string formatada
            ...
            # Limpar descrição (mantém descrição original por enquanto)
            data['instrucoesParticipante']['descricaoCasoCompleta'] = description
...
def main():
    """Função principal adaptada para análise prévia, correção e validação sem relatórios."""
    from glob import glob

    # Detectar automaticamente o diretório de execução e ajustar caminhos
    current_dir = os.getcwd()
    script_dir = os.path.dirname(os.path.abspath(__file__))

    # Verificar se estamos executando da pasta raiz ou da pasta 'correção nomes'
    if os.path.basename(current_dir) == "correção nomes":
        # Estamos na pasta 'correção nomes', ajustar caminhos para a pasta pai
        parent_dir = os.path.dirname(current_dir)
        target_directory = os.path.join(parent_dir, "ESTAÇÕES A SEREM AUDITADAS")
        names_file_path = os.path.join(parent_dir, "banco de nomes.json")
    else:
        # Estamos na pasta raiz, usar caminhos relativos
        target_directory = "ESTAÇÕES A SEREM AUDITADAS"
        names_file_path = "banco de nomes.json"

    print(f"Diretório atual: {current_dir}")
    print(f"Diretório do script: {script_dir}")
    print(f"Pasta alvo: {target_directory}")
    print(f"Arquivo de nomes: {names_file_path}")

    if not os.path.exists(target_directory):
        print(f"Diretório não encontrado: {target_directory}")
        return

    # Inicializar processador com caminho correto para o banco de nomes
    try:
        enricher = PatientDataCleaner(names_file_path)
    except Exception as e:
        print(f"Erro ao inicializar: {e}")
        return

    # 1. Análise prévia: identificar arquivos que precisam de alteração
    json_files = []
    ...
    for file_path in json_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            has_duplicates, identification_count = enricher._check_duplicate_identification(data)
            is_correct, _ = enricher._is_file_already_correct(data)
            if has_duplicates or not is_correct:
                arquivos_para_corrigir.append(file_path)
                analise_status[file_path] = "precisa_correção"
            else:
                analise_status[file_path] = "ok"
        except Exception as e:
            analise_status[file_path] = f"erro_analise: {e}"

    print(f"Total de arquivos analisados: {len(json_files)}")
    print(f"Arquivos que precisam de correção: {len(arquivos_para_corrigir)}")

    # 2. Processar apenas os arquivos identificados
    erros_processamento = []
    for file_path in arquivos_para_corrigir:
        success, msg = enricher.process_file(file_path)
        if not success:
            erros_processamento.append((file_path, msg))

    print(f"Arquivos corrigidos: {len(arquivos_para_corrigir) - len(erros_processamento)}")
    print(f"Arquivos com erro após tentativa de correção: {len(erros_processamento)}")

    # 3. Validação final: tentar corrigir novamente apenas os que deram erro
    if erros_processamento:
        print("Tentando corrigir novamente arquivos com erro...")
        erros_restantes = []
        for file_path, _ in erros_processamento:
            success, msg = enricher.process_file(file_path)
            if not success:
                erros_restantes.append((file_path, msg))
        print(f"Arquivos ainda com erro após segunda tentativa: {len(erros_restantes)}")
        if erros_restantes:
            for file_path, msg in erros_restantes:
                print(f"Erro em {file_path}: {msg}")
